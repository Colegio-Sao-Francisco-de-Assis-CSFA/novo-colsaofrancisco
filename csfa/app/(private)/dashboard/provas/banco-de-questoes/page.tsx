...
import { ChevronDown, Filter, ChevronRight, Search } from 'lucide-react';
import { Questao } from "@/types";
import QuestaoLista from '@/components/apadges/Questao/QuestaoLista';

async function getQuestoes(): Promise<Questao[]> {
  const res = await fetch(`/api/questoes/routes.ts`, {
    cache: "no-store",
  });
  return res.json();
}
...
export default async function BancoQuestoes() {
  ...
  return (
    ...
    <div className="p-6">
      ...
      <QuestaoLista questoes={questoes} />
    </div>
    ...
    //         {/* wrap-top */}
    //         <header className="bg-blue-300 w-full h-20 flex items-center justify-between px-6">
    ...
    //             <div>
    //                 <label htmlFor="buscar__questao" className="flex gap-2 items-center cursor-pointer">
    //                     <Search />
    //                     <input type="search" name="buscar__questao" id="buscar__questao" placeholder="Pesquise por assunto" />
    //                 </label>
    //             </div>
    ...
    //         {/* Sidebar Filters */}
    //         <aside className="w-64 border-r p-4 overflow-y-auto absolute z-10 bg-white">
